...
def test_login_success_with_2fa(browser_manager):
    ...
    mock_gmail_client.get_latest_verification_email_id.side_effect = ["old_email_id", "new_email_id"]
...
def test_login_2fa_code_expired(browser_manager):
    ...
    mock_gmail_client.get_verification_code_by_id.side_effect = VerificationCodeError("認証コードの有効期限が切れています")
...
def test_wait_for_new_verification_email_success(browser_manager):
    """新しい認証メール待機の成功テスト。"""
    mock_gmail_client = MagicMock(spec=GmailClient)
    mock_gmail_client.get_latest_verification_email_id.side_effect = ["old_id", "new_id"]
    
    result = browser_manager.wait_for_new_verification_email(mock_gmail_client, "old_id")
...
def test_wait_for_new_verification_email_api_error(browser_manager):
    """Gmail APIエラー時のテスト。"""
    mock_gmail_client = MagicMock(spec=GmailClient)
    mock_gmail_client.get_latest_verification_email_id.side_effect = GmailApiError("API Error")

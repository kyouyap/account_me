...
def test_create_gmail_service_missing_token(monkeypatch):
    """トークンが設定されていない場合のテスト。"""
    monkeypatch.setenv(
        "GMAIL_CREDENTIALS", json.dumps({"installed": {"client_id": "test"}})
    )
    ...
    with patch(
        "scraper.gmail_client.InstalledAppFlow.from_client_config",
...
def test_create_gmail_service_secret_update_failure(monkeypatch):
    ...
    with patch("scraper.gmail_client.Credentials") as mock_credentials:
        mock_credentials.from_authorized_user_info.return_value = old_creds
        with patch(
            "scraper.gmail_client.update_secret", side_effect=Exception("update failed")
        ):
            with patch("scraper.gmail_client.build"):
                with pytest.raises(GmailApiError, match="認証処理に失敗"):
                    GmailClient()
            ...
            with patch("scraper.gmail_client.update_secret") as mock_update:
                with patch("scraper.gmail_client.build") as mock_build:
                    ...
                    mock_update.assert_called_once_with(
                        "gmail-api-token", "refreshed_json"
                    )
                    mock_build.assert_called_once_with(
                        "gmail", "v1", credentials=old_creds
                    )
        ...
        with patch(
            ...
            with patch("scraper.gmail_client.update_secret") as mock_update:
                with patch("scraper.gmail_client.build") as mock_build:
                    ...
                    mock_update.assert_called_once_with(
                        "gmail-api-token", "newflow_json"
                    )
                    mock_build.assert_called_once_with(
                        "gmail", "v1", credentials=new_creds
                    )
...
def test_parse_message_invalid_base64(gmail_client):
    ...
    msg = {
        "payload": {
            "parts": [{"mimeType": "text/plain", "body": {"data": "invalid-base64"}}]
        }
    }
...
def test_get_latest_verification_email_id_network_error(gmail_client):
    """ネットワークエラーの場合のテスト。"""
    gmail_client.service.users.return_value.messages.return_value.list.side_effect = (
...
def test_get_verification_code_by_id_api_error(gmail_client):
    ...
    gmail_client.service.users.return_value.messages.return_value.get.side_effect = (
        HttpError(MagicMock(), b"err")
    )
    with pytest.raises(GmailApiError, match="Gmail APIのリクエストに失敗"):
    ...
    msg = {
        "payload": {
            "parts": [
                {
                    "mimeType": "text/plain",
                    "body": {"data": "VGVzdCBtZXNzYWdlCk5vIGNvZGUgaGVyZQ=="},
                }
            ]
        }
    }
...
def test_get_verification_code_api_error(gmail_client):
    ...
    gmail_client.service.users.return_value.messages.return_value.list.side_effect = (
        HttpError(MagicMock(), b"API Error")
    )
    with pytest.raises(GmailApiError, match="Gmail APIのリクエストに失敗"):
        gmail_client.get_verification_code()
...
def test_get_verification_code_execution_error(gmail_client):
    ...
    gmail_client.service.users.return_value.messages.return_value.list.return_value.execute.side_effect = Exception(
        "execution error"
    )
    with pytest.raises(GmailApiError, match="Gmail APIのリクエストに失敗"):
        gmail_client.get_verification_code()
...
def test_get_verification_code_by_id_invalid_message(gmail_client):
    """メッセージの形式が不正な場合のテスト。"""
    gmail_client.service.users.return_value.messages.return_value.get.return_value.execute.return_value = {
        "invalid": "format"
    }
    with pytest.raises(VerificationCodeError, match="メッセージの解析に失敗"):
        gmail_client.get_verification_code_by_id("id")
...
def test_get_verification_code_concurrent_error(gmail_client):
    ...
    gmail_client.service.users.return_value.messages.return_value.get.return_value.execute.side_effect = Exception(
        "concurrent error"
    )
...
def test_parse_message_recursive_error(gmail_client):
    ...
    msg = {
        "payload": {"parts": [{"mimeType": "text/plain", "body": {"data": "invalid"}}]}
    }
...
def test_parse_message_corrupted_data(gmail_client):
    ...
    msg = {
        "payload": {
            "parts": [
                {"mimeType": "text/plain", "body": {"data": "corrupted-base64-data"}}
            ]
        }
    }
    with pytest.raises(VerificationCodeError, match="メッセージの解析に失敗"):
...
def test_parse_message_invalid_structure(gmail_client):
    ...
    msg = {
        "payload": {
            "parts": [{"mimeType": "text/plain", "body": {"wrong_key": "data"}}]
        }
    }
    with pytest.raises(VerificationCodeError, match="メッセージの解析に失敗"):
        gmail_client._parse_message(msg)
...
def test_create_gmail_service_build_error(monkeypatch):
    ...
    monkeypatch.setenv(
        "GMAIL_CREDENTIALS",
        '{"installed":{"client_id":"a","project_id":"b","auth_uri":"x","token_uri":"y","client_secret":"z","redirect_uris":["u"]}}',
    )
    monkeypatch.setenv(
        "GMAIL_API_TOKEN", '{"token":"t","refresh_token":"r","client_id":"a"}'
    )
    # モック資格情報
...
def test_get_latest_verification_email_id_missing_branches(
    gmail_client, result, exc_type, match
):
    svc = (
        gmail_client.service.users.return_value.messages.return_value.list.return_value
    )
...
def test_get_verification_code_message_none(gmail_client):
    ...
    # get().execute() が None
    get_call = (
        gmail_client.service.users.return_value.messages.return_value.get.return_value
    )
...
def test_get_verification_code_by_id_success(gmail_client, sample_message):
    ...
    gmail_client.service.users().messages().get(
        return_value=MagicMock(execute=MagicMock(return_value=sample_message))
...
def test_get_verification_code_success(gmail_client, sample_message):
    ...
    gmail_client.service.users().messages().list(
        return_value=MagicMock(
            execute=MagicMock(return_value={"messages": [{"id": "any-id"}]})
        )
